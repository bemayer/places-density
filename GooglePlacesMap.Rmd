---
title: "Where to find the most Google Maps places in Parisian North-East ?"
description: "Geostatistics project for CNAM"
author: "Benoît Mayer"
output:
...
setwd("/Users/BeMayer/Google Drive/Projets dev/WordDensityMap/")
...
![Staggered grid](staggered_grid.png){width=100%}

Let's consider the superposition between two circles. 

![Circle superposition](circle_superposition.png){width=100%}
...
The running time of process was 14 hours and it did not meet any issue.
...
places <- do.call(rbind, c(lapply(files, fread), fill = TRUE)) %>% as_tibble()
places
...
``` {r}
rmarkdown::paged_table(places %>% as_tibble() %>% dplyr::select(-Id) %>% mutate(Name = str_trunc(Name, 30, "right"), Type = str_trunc(Type, 50, "right")))
...
I also removed the duplicates, transformed the `places` object to an sf object, arranged it by the number of ratings of each places and filtered to keep only the locations in the limits of the North-East arrondissements.
...
- Faubourg-Montmartre is the district with the greater density of restaurants. It is possible to see tens of them just by wandering on rue du Faubourg Montmartre, including very famous ones such as Le Bouillon Chartier.
